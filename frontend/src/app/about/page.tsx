...
async function getAboutContent(): Promise<AboutPageData | null> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `Failed to fetch About Page: ${res.status} ${res.statusText}`
      );
      return null;
    }
    ...
    console.log(
      'Fetched About Page data:',
      pageData ? '[Data Object]' : pageData
    ); // FIXME: Optional: for debugging
...
export default async function AboutPage() {
  ...
  if (!content) {
    return (
      <main className="mx-auto max-w-3xl p-4 text-center text-red-500">
  ...
  return (
    <main className="mx-auto max-w-3xl p-4">
      <h1 className="mb-6 text-3xl font-bold">{content.title || 'About Me'}</h1>
